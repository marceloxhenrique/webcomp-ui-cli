...
export function setupFrameork(
  selectedFramework: number,
  frameworkList: { framework: string; color: number }[]
) {
  console.log(
  ...
  console.log("You can now run: npx webcomp-ui-cli add <component>");
  fs.writeFileSync(
    CONFIG_PATH,
    JSON.stringify({ framework: frameworkList[selectedFramework].framework }, null, 2)
  );
